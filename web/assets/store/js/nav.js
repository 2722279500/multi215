(function () {
     ...
     * @constructor
     */
    function diyPhone(initalData, diyData) {
            ...
            new Vue({
                ...
                methods: {
                    ...
                    onEditorAddData: function () {
                        // 新增data数据
                        if (this.diyData.items[0].data.length >= 5) {
                            layer.msg('最多添加5个', {anim: 6});
                            return false;
                        }
                        var newDataItem = $.extend(true, {}, defaultData['items'][0].data[0]);
                    ...
                    onSubmit: function () {
                        ...
                        if (this.diyData.items.length >= 5) {
                            layer.msg('最多只能添加5个', {anim: 6});
                            return false;
                        }

                        var data = JSON.stringify(this.diyData);
                        var timeStamp = parseInt(new Date().getTime() / 1000);
                        localStorage.setItem("diy_nav_" + timeStamp, data);
