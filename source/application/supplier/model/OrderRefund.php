...
class OrderRefund extends OrderRefundModel
{
    ...
    public function getList($query = [])
    {
        ...
        return $this->alias('m')
            ...
            ->where(['m.supplier_id'=>$user['supplier_user_id']])
            // ->join('order_goods', 'order_goods.order_id = m.order_id AND order_goods.supplier_id='.$user['supplier_user_id'])
            ->order(['m.create_time' => 'desc'])
            // ->group("m.order_refund_id")
            ->paginate(10, false, [
    ...
    public function audit($data)
    {
        ...
        $this->transaction(function () use ($data) {
            ...
            // 同意售后申请, 记录退货地址
            if ($data['is_agree'] == 10) {
                $type = \think\Db::name("order_refund")->where(['order_refund_id'=>$this['order_refund_id']])->value("type");
                if($type == 10)
                {
                    if(false == citrixAddSupplierRefund($this['order_refund_id']))
                    {
                        $this->error = '请勿重复操作';
                        return false;
                    }
