...
use app\supplier\model\User as UserModel;
use app\supplier\model\UserCoupon as UserCouponModel;
use app\supplier\service\order\Export as Exportservice;
...
class Order extends OrderModel
{
    ...
    public function getList($dataType, $query = [])
    {
        ...
        // $list = $this->with(['goods.image', 'address', 'user'])
        //     ->alias('order')
        //     ->field('order.*')
        //     ->join('user', 'user.user_id = order.user_id')
        //     // ->join('goods', 'goods.supplier_id = '.self::$supplier_id)
        //     ->where($this->transferDataType($dataType))
        //     ->where('order.is_delete', '=', 0)
        //     ->where('order.supplier_id',"like",'%:'.self::$supplier_id.'}%')
        ...
        //     {
        //         $SupplierLogModel = new SupplierLogModel($item['order_id']);
        //         $item['gross_profit'] = $SupplierLogModel->getBalanceLog(self::$supplier_id);
        //         return $item;
        //     });
        // p($list);
        // exit;
     ...
     * @throws \think\exception\DbException
     */
    public function getFinanceList($dataType, $query = [])
    {
        ...
        return $this->with(['goods.image', 'address', 'user'])
            ->alias('order')
            ->field('`order`.*,`order_goods`.express_price')
            ->join('user', 'user.user_id = order.user_id')
            ->join('order_goods', 'order_goods.supplier_id = '.self::$supplier_id)
            ->where($this->transferDataType($dataType))
            ->where('order.is_delete', '=', 0)
    ...
    private function updateToDelivery($orderList)
    {
        ...
        foreach ($orderList as $item) 
        {
            if(true == citrixCheckPartialShip(self::$supplier_id,$item['order_id']))
            {
                $attr = [
                ...
                if($list = citrixGetShipPrice(self::$supplier_id,$item['order_id']))
                {
                ...
                $this->citrixExpressPrice($item['order_id'],self::$supplier_id);
            }else
            {
                $attr = [
                    'order_id' => $item['order_id'],
     ...
     * @return bool
     */
    private function citrixExpressPrice($order_id,$supplier_id)
    {
        
       /* $list = Db::name("order_goods")->field("order_goods_id,express_price,total_price")->where(['order_id'=>$order_id,'supplier_id'=>$supplier_id])->select()->toArray();
        $count = count($list);
        $sum = Db::name("order_goods")->field("sum(total_price) as total_price")->where(['order_id'=>$order_id,'supplier_id'=>$supplier_id])->select()->toArray();
        $sum = $sum[0]['total_price'];

        foreach ($list as $k1 => $v1) 
        {
            $freight = $list[$k1]['total_price']/$sum*"100%";

            // p($list[$k1]['total_price']);
            // p($freight."%");
            echo "-----------------";
            p($list[$k1]['express_price']/$freight);
            echo "++++++++++++++++++++";
        }
*/
        // echo "++++++++++++++++++++";
