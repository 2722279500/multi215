...
use app\supplier\model\dealer\User as DealerUserModel;
use app\supplier\model\user\GradeLog as GradeLogModel;
...
class User extends UserModel
{
    ...
    /**
     * 用户充值
     * @param string $supplierUserName 当前操作人用户名
     * @param int $source 充值类型
     * @param array $data post数据
     * @return bool
     */
    public function recharge($supplierUserName, $source, $data)
    {
        if ($source == 0) {
            return $this->rechargeToBalance($supplierUserName, $data['balance']);
        } elseif ($source == 1) {
            return $this->rechargeToPoints($supplierUserName, $data['points']);
        }
        return false;
    }
     ...
     * @return bool
     */
    private function rechargeToBalance($supplierUserName, $data)
    {
        ...
        // 更新记录
        $this->transaction(function () use ($supplierUserName, $data, $diffMoney) {
            // 更新账户余额
     ...
     * @return bool
     */
    private function rechargeToPoints($supplierUserName, $data)
    {
        ...
        $this->transaction(function () use ($supplierUserName, $data, $diffMoney) {
            ...
            PointsLogModel::add([
                'user_id' => $this['user_id'],
                'value' => $diffMoney,
                'describe' => "后台管理员 [{$supplierUserName}] 操作",
                'remark' => $data['remark'],
            ]);
        });
        return true;
