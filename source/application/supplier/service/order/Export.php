...
use app\supplier\model\OrderAddress as OrderAddressModel;
use app\api\model\SupplierLog as SupplierLogModel;

/**
 * 订单导出服务类
 * Class Export
 * @package app\supplier\service\order
 */
class Export
{
    /**
     ...
     */
    private $tileArray = [
        '订单号', '商品信息','订单详情', '订单总额', '优惠券抵扣', '积分抵扣', '运费金额', '后台改价', '实付款金额', '支付方式', '下单时间',
    ...
    public function orderList($list)
    {
        ...
        $supplier_id = $yoshop_supplier['supplier_user_id'];
        $m = \think\Db::name("order_goods");
        // 表格内容
        $dataArray = [];
        foreach ($list as $order) {
            /* @var OrderAddressModel $address */
            $address = $order['address'];
            $supplierLogModel = new SupplierLogModel($order['order_id']);
            $order['gross_profit'] = $supplierLogModel->getBalanceLog($supplier_id);
            $dataArray[] = [
                '订单号' => $this->filterValue($order['order_no']),
                '商品信息' => $this->filterGoodsInfo($order),
                '订单详情' => $this->filterValue(implode("\r\n\t",$order['gross_profit']['describe'])),
                ...
                '物流公司' => $this->filterValue($m->where(['order_id'=>$order['order_id'],'supplier_id'=>$supplier_id])->value("express_company")),
                '物流单号' => $this->filterValue($m->where(['order_id'=>$order['order_id'],'supplier_id'=>$supplier_id])->value("express_no")),
    ...
    private function filterGoodsInfo($order)
    {
        ...
        {
            if($yoshop_supplier['supplier_user_id'] == $order['goods'][$key]['supplier_id'])
            {
