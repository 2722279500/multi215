...
use app\supplier\model\supplier\Access;
use think\Session;
use app\supplier\model\supplier\User;
use app\supplier\model\supplier\UserRole;
use app\supplier\model\supplier\RoleAccess;
...
class Auth
{
    ...
    private function __construct()
    {
        ...
        if (!empty($this->supplier)) {
            $this->user = User::detail($this->supplier['user']['supplier_user_id']);
        }
    }
    ...
    private function getAccessUrls()
    {
        if (empty($this->accessUrls)) {
            // 获取当前用户的角色集
            $roleIds = UserRole::getRoleIds($this->user['supplier_user_id']);
            // 根据已分配的权限
