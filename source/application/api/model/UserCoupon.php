...
class UserCoupon extends UserCouponModel
{
     ...
     * @throws \think\exception\DbException
     */
    public function citrix_receive($user, $coupon_id,$wxapp_id)
    {
        // 获取优惠券信息
        $coupon = Coupon::detail($coupon_id);
        // 验证优惠券是否可领取
        if (false == $this->citrix_checkReceive($user, $coupon,$wxapp_id)) 
        {
            return false;
        }
        // 添加领取记录
        return $this->citrix_add($user, $coupon,$wxapp_id);
    }
     ...
     * @return bool
     */
    private function citrix_checkReceive($user, $coupon,$wxapp_id)
    {
        if (!$coupon) {
            return false;
        }
     ...
     * @return bool
     */
    private function citrix_add($user, $coupon,$wxapp_id)
    {
        // 计算有效期
